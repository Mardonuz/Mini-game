import socket, threading, base64, hashlib, json, hmac, time, os
        ...
        while True:
            ...
            if opcode == 0x1:
                ...
                if t == "join":
                    ...
                    if not verify(token, pl_str):
                        send_text(conn, json.dumps({"t": "error", "m": "Invalid or expired token"}))
                        continue
                    
                    try:
                        pl_obj = json.loads(pl_str)
                        exp = pl_obj.get("exp", 0)
                    except:
                        exp = 0
...
def run():
    ...
    host = "0.0.0.0"
    print(f"[WS] Server starting on {host}:{port}...")
