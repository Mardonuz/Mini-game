export class SpriteAtlas {
  ...
  _drawFallback(g, kind) {
    if (kind === "knight") {
      ...
      g.fillStyle = "#ffd700";
      g.beginPath();
      g.moveTo(10, 10); g.lineTo(15, 5); g.lineTo(20, 10); g.lineTo(25, 5); g.lineTo(30, 10);
      g.fill();
  ...
  drawTower(ctx, tower, color, kind = "tower") {
    const f = this.frame(kind, 0);
    const w = tower.w || 30;
    const h = tower.h || 45;
    ctx.save();
  ...
  drawEffect(ctx, ef) {
    const f = this.frame(ef.kind, performance.now() * 0.001);
    ctx.save();
