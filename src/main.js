...
async function start() {
  ...
  if (multiplayer && net && net.connected) {
    ...
    const pl = { cid: net.cid, exp };
    const token = await signToken(pl);
    net.join(room, JSON.stringify(pl), token);
    net.send({ t: "list" });
  }
...
testBtn.addEventListener("click", () => {
  runBalanceTests();
  alert("Balans testlari konsolga chiqarildi (F12)!");
});
